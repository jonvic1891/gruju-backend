...
router.get('/search', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const users = await DatabaseHelper.getMany(
    ...
    const usersWithChildren = await Promise.all(
      users.map(async (user) => {
        const children = await DatabaseHelper.getMany(
          'SELECT id, name FROM Children WHERE parent_id = @parentId',
          { parentId: user.id }
        );
...
router.post('/request', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const ownChild = await DatabaseHelper.getOne(
      'SELECT id FROM Children WHERE id = @childId AND parent_id = @userId',
      { childId: child_id, userId }
    );
    ...
    const targetParent = await DatabaseHelper.getOne(
      'SELECT id FROM Users WHERE id = @targetParentId',
    ...
    if (target_child_id) {
      const targetChild = await DatabaseHelper.getOne(
        'SELECT id FROM Children WHERE id = @targetChildId AND parent_id = @targetParentId',
        { targetChildId: target_child_id, targetParentId: target_parent_id }
      );
    ...
    const existingConnection = await DatabaseHelper.getOne(
    ...
    const existingRequest = await DatabaseHelper.getOne(
      `SELECT id FROM ConnectionRequests 
    ...
    const requestId = await DatabaseHelper.insertAndGetId(
      `INSERT INTO ConnectionRequests (requester_id, target_parent_id, child_id, target_child_id, message, status) 
    ...
    const newRequest = await DatabaseHelper.getOne(
      `SELECT cr.*, u.username as requester_name, c.name as child_name,
...
router.get('/requests', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const requests = await DatabaseHelper.getMany(
...
router.post('/requests/:requestId/accept', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    const { requestId } = req.params;

    // Get the request and verify ownership
    const request = await DatabaseHelper.getOne(
      'SELECT * FROM ConnectionRequests WHERE id = @requestId AND target_parent_id = @userId AND status = \'pending\'',
      { requestId: parseInt(requestId), userId }
    );
    ...
    await DatabaseHelper.executeQuery(
      'INSERT INTO Connections (child1_id, child2_id, status) VALUES (@child1Id, @child2Id, \'active\')',
      {
    ...
    await DatabaseHelper.executeQuery(
      'UPDATE ConnectionRequests SET status = \'accepted\', updated_at = GETUTCDATE() WHERE id = @requestId',
...
router.post('/requests/:requestId/reject', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    const { requestId } = req.params;
    ...
    // Update request status
    await DatabaseHelper.executeQuery(
      'UPDATE ConnectionRequests SET status = \'rejected\', updated_at = GETUTCDATE() WHERE id = @requestId',
      { requestId: parseInt(requestId) }
    );
...
router.get('/', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const connections = await DatabaseHelper.getMany(
      `SELECT DISTINCT conn.id, conn.created_at,
...
router.delete('/:connectionId', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const connection = await DatabaseHelper.getOne(
      `SELECT conn.* FROM Connections conn
       ...
       WHERE conn.id = @connectionId 
         ...
         AND conn.status = 'active'`,
      { connectionId: parseInt(connectionId), userId }
    );
    ...
    await DatabaseHelper.executeQuery(
      'UPDATE Connections SET status = \'deleted\', updated_at = GETUTCDATE() WHERE id = @connectionId',

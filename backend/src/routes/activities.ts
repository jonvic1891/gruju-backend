...
router.get('/', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (child_id) {
      ...
      params.child_id = parseInt(child_id as string);
    }

    query += ` ORDER BY a.start_date DESC, a.start_time DESC`;

    const activities = await DatabaseHelper.getMany(query, params);
...
router.get('/:activityId', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
...
// Update an activity
router.put('/:activityId', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    const { activityId } = req.params;
    ...
    const existingActivity = await DatabaseHelper.getOne(
    ...
    const updatedActivity = await DatabaseHelper.getOne(
...
// Delete an activity
router.delete('/:activityId', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    const { activityId } = req.params;
    ...
    // Check if activity exists and user owns the child
    const existingActivity = await DatabaseHelper.getOne(
      `SELECT a.id, a.child_id FROM Activities a
       INNER JOIN Children c ON a.child_id = c.id
       WHERE a.id = @activityId AND c.parent_id = @userId`,
      { activityId: parseInt(activityId), userId }
    );
...
router.post('/:activityId/duplicate', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    const { activityId } = req.params;
    ...
    const originalActivity = await DatabaseHelper.getOne(
    ...
    const newActivityId = await DatabaseHelper.insertAndGetId(
    ...
    const newActivity = await DatabaseHelper.getOne(
      'SELECT id, name, start_date, end_date, start_time, end_time, website_url, created_at, updated_at FROM Activities WHERE id = @newActivityId',
      { newActivityId }
    );

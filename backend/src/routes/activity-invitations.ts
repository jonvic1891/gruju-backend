...
router.get('/', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const invitations = await DatabaseHelper.getMany(
      `SELECT ai.id, ai.activity_id, ai.inviter_parent_id, ai.invited_parent_id, 
...
router.get('/sent', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;

    const sentInvitations = await DatabaseHelper.getMany(
      `SELECT ai.id, ai.activity_id, ai.inviter_parent_id, ai.invited_parent_id, 
              ai.child_id, ai.status, ai.message, ai.created_at, ai.updated_at,
              a.name as activity_name, a.description as activity_description,
...
router.post('/:invitationId/respond', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;
    ...
    const invitation = await DatabaseHelper.getOne(
      `SELECT ai.id, ai.activity_id, ai.status, a.name as activity_name
       FROM activity_invitations ai
       INNER JOIN Activities a ON ai.activity_id = a.id
       WHERE ai.id = @invitationId AND ai.invited_parent_id = @userId`,
      { invitationId: parseInt(invitationId), userId }
    );
...
router.get('/counts', async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.id;

    const counts = await DatabaseHelper.getOne(
      `SELECT 
         COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending_count,
         COUNT(CASE WHEN status = 'accepted' THEN 1 END) as accepted_count,
         COUNT(CASE WHEN status = 'rejected' THEN 1 END) as rejected_count
       FROM activity_invitations 
       WHERE invited_parent_id = @userId`,
      { userId }
    );

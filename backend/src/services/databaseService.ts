...
export interface DatabaseConfig {
  ...
  user: string;
  password: string;
  environment: 'mock' | 'demo' | 'test' | 'production';
  name: string;
  description?: string;
  options?: {
    encrypt?: boolean;
...
export class DatabaseService {
  ...
  async addDatabaseConfig(configId: string, config: DatabaseConfig): Promise<boolean> {
    try {
  ...
  getDatabaseConfigs(): Map<string, DatabaseConfig> {
    return new Map(this.databaseConfigs);
  }

  getDatabaseConfig(configId: string): DatabaseConfig | undefined {
  ...
  async setAzureConfig(config: DatabaseConfig): Promise<boolean> {
    try {
      const testPool = new sql.ConnectionPool(config);
      await testPool.connect();
      await testPool.request().query('SELECT 1');
  ...
  async switchToEnvironment(environmentId: string): Promise<boolean> {
  ...
  async switchToAzure(): Promise<boolean> {
  ...
  getCurrentMode(): 'mock' | 'demo' | 'test' | 'production' {
    return this.currentMode;
  }

  // Initialize Azure SQL tables
  private async initializeAzureTables(): Promise<void> {
    if (!this.connectionPool) throw new Error('No Azure connection');
  ...
  async createUser(userData: Omit<User, 'id' | 'created_at' | 'updated_at'>): Promise<User> {
    ...
    const result = await this.connectionPool.request()
      ...
      .input('family_name', sql.NVarChar, userData.family_name)
      .query(`
  ...
  async updateUserRole(userId: number, role: 'user' | 'admin' | 'super_admin'): Promise<User | null> {
    if (this.currentMode === 'mock') {
  ...
  async updateUserStatus(userId: number, isActive: boolean): Promise<User | null> {
    if (this.currentMode === 'mock') {
  ...
  // Migration methods
  async migrateFromMockToAzure(): Promise<{ success: boolean; message: string; details?: any }> {
    if (this.currentMode === 'mock') {

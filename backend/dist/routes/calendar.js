...
router.get('/activities', async (req, res) => {
    try {
        ...
        const activities = await database_1.DatabaseHelper.getMany(`SELECT a.id, a.name, a.start_date, a.end_date, a.start_time, a.end_time, 
...
router.get('/connected-activities', async (req, res) => {
    try {
        const userId = req.user?.id;
        const { start, end } = req.query;
        if (!start || !end) {
        ...
        const connectedActivities = await database_1.DatabaseHelper.getMany(`SELECT DISTINCT a.id, a.name, a.start_date, a.end_date, a.start_time, a.end_time, 
              a.website_url, a.created_at, a.updated_at,
              c.name as child_name, c.id as child_id,
...
router.get('/activity-counts', async (req, res) => {
    try {
        const userId = req.user?.id;
    ...
    `;
      ...
      `;
        ...
        const activityCounts = await database_1.DatabaseHelper.getMany(query, {
            userId,
            start_date: start,
            end_date: end
        });
...
router.get('/activities/:date', async (req, res) => {
    try {
        ...
        let ownActivities = await database_1.DatabaseHelper.getMany(`SELECT a.id, a.name, a.start_date, a.end_date, a.start_time, a.end_time, 
              a.website_url, a.created_at, a.updated_at,
              c.name as child_name, c.id as child_id,
       ...
       WHERE c.parent_id = @userId 
         ...
         AND a.end_date >= @date
       ORDER BY a.start_time`, { userId, date });
